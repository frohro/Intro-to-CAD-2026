...
| OLED display | SSD1306 on same I²C bus; displays frequency and PLL parameters when connected |
...
The firmware automatically detects and uses an SSD1306 OLED display (128x64 pixels) connected to the same I²C bus as the MS5351M clock generator. The display shows:

- Current frequency in MHz (e.g., "Freq: 7.074 MHz")
- PLL parameters: M (PLL multiplier) and N (output divider)

The display is only updated when the OLED is physically connected; if disconnected, the firmware continues to operate normally without errors.
...
```
MicroPythonSDR/
├── 2026_Board/      MicroPython files for Intro-to-CAD-2026 board
│   ├── main.py      Entry point
│   ├── config.py    Board configuration (set to INTRO_CAD_2026)
│   ├── si5351.py    Si5351/MS5351M driver
│   └── radio.py     Radio class
├── Cadyn_Board/     MicroPython files for Cadyn's Pico SDR
│   ├── main.py      Entry point
│   ├── config.py    Board configuration (set to CADYN_PICO_SDR)
│   ├── si5351.py    Si5351/MS5351M driver
│   └── radio.py     Radio class
├── quisk_conf_2026.py     Quisk configuration for 2026 board
├── quisk_conf_cadyn.py    Quisk configuration for Cadyn's board
└── README.md              This file
```
...
### MicroPython Firmware for the Pico
...
- For the **Intro-to-CAD-2026 board**: Use files from `2026_Board/`
- For **Cadyn's Pico SDR**: Use files from `Cadyn_Board/`

#### 3. Copy MicroPython files to the Pico

Using **MicroPico** (VS Code extension) or **Thonny**:

Copy these four files from your chosen board directory to the root (`/`) of the Pico filesystem:
...
### Quisk SDR Software Setup

#### 1. Install Quisk

Install Quisk on your computer (Linux recommended):

```bash
sudo apt install quisk  # Ubuntu/Debian
# or
pip install quisk       # from PyPI
```

#### 2. Install Quisk configurations

Copy the appropriate Quisk configuration file to your Quisk config directory:

```bash
# For 2026 board:
cp quisk_conf_2026.py ~/.quisk/

# For Cadyn's board:
cp quisk_conf_cadyn.py ~/.quisk/
```

#### 3. Run Quisk with hardware selection

Launch Quisk and select your radio from the menu:

```bash
quisk -a
```

This opens Quisk with a menu to choose between available hardware configurations. Select the appropriate one for your connected board.

The configurations are named to distinguish between the two radios, allowing you to switch between them easily.
