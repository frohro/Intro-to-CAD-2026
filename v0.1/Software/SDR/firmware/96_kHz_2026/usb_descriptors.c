/*
 * usb_descriptors.c — Research: 48/96 kHz Dual-Rate UAC1 SDR
 *
 * Changes from Lab 7:
 *
 *   bSubframeSize: 4 → 3   (S24_3LE — true 24-bit, 3 bytes per sample)
 *   bBitResolution: 24      (unchanged)
 *   bSamFreqType: 2 → 1 per alt  (separate alt settings per rate)
 *   Alt 1: wMaxPacketSize = 294, 48 kHz only  (fits EHCI Single-TT)
 *   Alt 2: wMaxPacketSize = 582, 96 kHz only
 *   AUDIO_BLOCK_LEN: 112 → 155  (added alt 2 block, FORMAT_LEN(2)→FORMAT_LEN(1)×2)
 *   CONFIG_TOTAL_LEN: 187 → 230
 *
 ...
 * └────────────────────────────────────────────────────────────────────┘
 *
 * AUDIO_BLOCK_LEN breakdown (UAC1, two alt settings with 1 freq each):
 *   Manual Audio IAD                                =  8
 ...
 *
 * Audio:
 *   0x81 — EP1 IN  isochronous async     392 B   stereo 24-in-32 PCM data
 *
...
static const uint8_t desc_configuration[] = {
    ...
    /* Standard AS Interface — alt 1: 48 kHz active (1 isochronous EP) */
    TUD_AUDIO10_DESC_STD_AS_INT(/*_itfnum*/ ITF_NUM_AUDIO_STREAMING,
                                 /*_altset*/ 0x01,
                                 /*_nEPs*/   0x01,
                                 /*_stridx*/ 0x00),
     ...
     *   bSamFreqType = 1, tSamFreq[0] = 48000 = 0x00BB80 */
    11,                    /* bLength                  */
     ...
     * 48000 * 3 B/sample * 2 ch / 1000 = 288 B + 6 B (±1 sample headroom) = 294.
     * 294 bytes fits comfortably through an EHCI Single-TT hub. */
    ...
    /* Standard AS Interface — alt 2: 96 kHz active (1 isochronous EP) */
    TUD_AUDIO10_DESC_STD_AS_INT(/*_itfnum*/ ITF_NUM_AUDIO_STREAMING,
                                 /*_altset*/ 0x02,
                                 /*_nEPs*/   0x01,
                                 /*_stridx*/ 0x00),
     ...
     * Wire layout (11 bytes = 8 fixed + 3 * 1 freq):
     *   bSamFreqType = 1, tSamFreq[0] = 96000 = 0x017700 */
    11,                    /* bLength                  */
    TUSB_DESC_CS_INTERFACE, /* bDescriptorType          */
    0x02,                  /* bDescriptorSubtype: FORMAT_TYPE */
    0x01,                  /* bFormatType: TYPE_I      */
     ...
     * 96000 * 3 B/sample * 2 ch / 1000 = 576 B + 6 B (±1 sample headroom) = 582. */
    TUD_AUDIO10_DESC_STD_AS_ISO_EP(/*_ep*/       EPNUM_AUDIO_IN,
                                    /*_attr*/     (uint8_t)(TUSB_XFER_ISOCHRONOUS |
                                                            TUSB_ISO_EP_ATT_ASYNCHRONOUS),
                                    /*_maxEPsize*/ 582,
