...
# Check bSamFreqType = 1 (Single discrete frequency advertised)
samfreq_matches = re.findall(r"bSamFreqType\s+(\d+)", lsusb_out)
n_samfreq1 = samfreq_matches.count("1")
if n_samfreq1 == 1:
    print(f"  {PASS}  Exactly one bSamFreqType=1 found (Single-rate mode)")
else:
    print(f"  {FAIL}  Expected exactly one bSamFreqType=1, got: {samfreq_matches}")
    failures += 1
...
# Check wMaxPacketSize: 582 B for 96 kHz S24_3LE
has_582 = bool(re.search(r"\b582\b", lsusb_out))
check("wMaxPacketSize = 582 B (96 kHz)", has_582, True)
...
# ---------------------------------------------------------------------------
# Part 5 -- Audio capture at 96 kHz
# ---------------------------------------------------------------------------
print()
print("=" * 60)
print("Part 5 -- Audio capture at 96 kHz S24_3LE (1 second)")
print("=" * 60)
...
if card_num is None:
    ...
    # With the single-alt design, we only test 96 kHz.
    ...
    MIN_96K = 500_000  # 1 s at 96 kHz stereo S24_3LE = 576000 bytes + 44 header
