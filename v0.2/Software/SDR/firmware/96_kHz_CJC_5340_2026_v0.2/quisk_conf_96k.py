...
# Research version: 48/96 kHz dual-rate (S24_3LE)
#
# The sample rate is selected in Quisk via Config -> Radio -> Sound.
# The firmware advertises both 48000 Hz and 96000 Hz; snd-usb-audio
# negotiates the rate with the device via UAC1 SET_CUR when Quisk
# opens the audio stream.  No changes to this file are needed.
...
name_of_sound_capt = "alsa:WWU SDR USB Audio (hw:3,0)"  # PortAudio ALSA device name as shown in Quisk Config -> Radio -> Sound
                                                        # The hw:X,0 card number varies by machine; select the correct
                                                        # device in Quisk's Config screen and it will update this value.
name_of_sound_play = "pulse"    # Playback through PipeWire/PulseAudio

sample_rate = 96000

# At 96 kHz Quisk calls read_sound() 200×/second (vs 100× at 48 kHz),
# doubling DSP+FFT load.  Occasional processing spikes exceed the default
# 150 ms output buffer, causing underruns (the "metronome" pulsing visible
# as errors on the Radio Sound Output row in the Status tab).
# 500 ms gives PulseAudio/PipeWire headroom for transient CPU spikes.
lin_latency_millisecs = 500
...
class Hardware(BaseHardware):
    ...
    def _candidate_for_lo(self, tune, lo_hz):
        ...
        best_n = None
        exact_m = None
        n_min = max(6, int(math.ceil(600_000_000.0 / lo_hz)))
        n_max = min(126, int(900_000_000.0 / lo_hz))

        for n_cand in range(n_min if n_min % 2 == 0 else n_min + 1, n_max + 1, 2):
            m_float = lo_hz * n_cand / crystal
        ...
        n_value = n_min if n_min % 2 == 0 else n_min + 1
        frac = fractions.Fraction(int(lo_hz * n_value), int(crystal)).limit_denominator(1048575)
        a_value = int(frac)
    ...
    def _find_integer_candidates(self, tune):
        ...
        results = []
        for n_cand in range(n_min if n_min % 2 == 0 else n_min + 1, n_max + 1, 2):
            m_float = tune * n_cand / crystal
            if not (14 < m_float < 91):
                continue
            m_int = round(m_float)
            lo = crystal * m_int / n_cand
            signed = lo - tune
            if abs(signed) < half_bw:
                candidate = self._candidate_from_values(tune, lo, n_cand, m_int, 0, 1)
    ...
    def _program_candidate(self, candidate, label):
        tune = candidate["lo"]
        self._send(
            f"FREQ,{tune},{candidate['n']},{candidate['a']},{candidate['b']},{candidate['c']},{candidate['p1']},{candidate['p2']},{candidate['p3']}"
        )
        self._readline()
