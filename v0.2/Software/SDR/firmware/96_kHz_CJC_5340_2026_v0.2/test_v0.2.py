...
target_rate = 96
target_hz = 96000
print(f"Testing for {target_rate} kHz configuration (fixed)...")
...
if port is None:
    for p in ("/dev/ttyACM0", "/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3"):
        try:
            ...
            print(f"  Opened {p}")
            # Try to guess PID from /sys or just use 4011 as fallback
...
# Use the actual VID:PID found in Part 1
...
# v0.2 Fixed-Rate descriptor has exactly ONE active alt setting (alt 1)
samfreq_matches = re.findall(r"bSamFreqType\s+(\d+)", lsusb_out)
n_samfreq1 = samfreq_matches.count("1")
check(f"Exactly one bSamFreqType=1 found (Fixed-rate mode)", n_samfreq1, 1)
...
# Check wMaxPacketSize: 582 B for 96 kHz
expected_max_packet = 582
# Find all wMaxPacketSize lines
all_max_packets = re.findall(r"wMaxPacketSize\s+0x[0-9a-fA-F]+\s+.*?\b(\d+)\b\s+bytes", lsusb_out)
# The first one is usually CDC (8 or 64), we want the large audio one (294)
got_max_packet = 0
...
def check_audio(label, wav_path, min_size, sample_window):
    ...
    if unique == 1 and samples[0] == 0:
        print(f"  {WARN}  Audio all-zero -- CJC5340 not producing samples.")
        print(f"         Check: GPIO6=DATA, GPIO7=BCK, GPIO8=WS connected;")
        print(f"         CJC5340 SCKI present; /RST wired HIGH.")
