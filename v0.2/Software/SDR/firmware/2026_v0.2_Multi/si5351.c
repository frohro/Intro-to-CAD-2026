...
bool si5351_init(i2c_inst_t *i2c)
{
    ...
    // CLK0: Raw 24.576 MHz crystal clock (Master Clock for ADC).
    // CLK2: Tunable LO (feeds Johnson counter for I/Q).
    
    // Enable CLK0 to output the raw 24.576 MHz crystal clock.
    // CLK0_CTRL (reg 16): Bit 7=0(PDN), Bits 5:4=01(XTAL), bits 1:0=11(8mA).
    si5351_write_reg(i2c, SI5351_REG_CLK0_CTRL, 0x13);
    ...
    // Enable CLK0 and CLK2 outputs (reg 3 bits 0, 2 = 0).
    si5351_write_reg(i2c, SI5351_REG_OEB, 0xFA); // 1111 1010: CLK0, CLK2 on
...
uint32_t si5351_set_freq_integer(i2c_inst_t *i2c, uint32_t freq_hz)
{
    ...
    // -----------------------------------------------------------------------
    // 3. Program Multisynth (MS0/1 for v0.1 DIRECT, MS2 for v0.2 JOHNSON).
    // -----------------------------------------------------------------------
    uint8_t ms_regs[8];
    pack_ms_int(ms_regs, best_M);

    // v0.2 defaults to JOHNSON mode (BOARD_DIRECT_MODE false in main.c).
    // In this mode, CLK2 feeds the external Johnson counter.
    si5351_write_reg(i2c, SI5351_REG_CLK2_CTRL, CLK_CTRL_INT_PLLA);
    write_reg_block(i2c, SI5351_REG_MS2_BASE, ms_regs);
    si5351_write_reg(i2c, SI5351_REG_CLK2_PHOFF, 0);
    ...
    si5351_write_reg(i2c, SI5351_REG_OEB, 0xFA);   // CLK0, CLK2 on
                          ...
                          bool direct_mode)
{
    ...
    if (direct_mode) {
        // v0.1 DIRECT mode: CLK0=I, CLK1=Q
        ...
        // v0.2 JOHNSON mode: CLK2=LO input (CLK0=ADC SCKI)
        si5351_write_reg(i2c, SI5351_REG_CLK2_CTRL, CLK_CTRL_INT_PLLA);
        write_reg_block(i2c, SI5351_REG_MS2_BASE, ms_regs);
        si5351_write_reg(i2c, SI5351_REG_CLK2_PHOFF, 0);
    }
