...
# Dynamically locate the WWU SDR ALSA card number to bypass pulseaudio/pulse resampling
card_num = None
try:
    result = subprocess.run(["arecord", "-l"], capture_output=True, text=True, timeout=1.0)
...
if card_num is not None:
    name_of_sound_capt = f"plughw:{card_num},0"
    print(f"Dynamically selected capture device: {name_of_sound_capt}")
else:
    name_of_sound_capt = "pulse"
    print("WWU SDR ALSA device not found, falling back to pulse")
...
class Hardware(BaseHardware):
    ...
    def sample_rate(self):
        return getattr(self.conf, "sample_rate", sample_rate)
    ...
    def open(self):
        ...
        # Force plughw instead of hw to support automatic format/rate conversion.
        # Strip "alsa:" and PortAudio display decoration to use direct ALSA PCM device name.
        if hasattr(self.conf, "name_of_sound_capt"):
            orig = self.conf.name_of_sound_capt
            import re
            m = re.search(r"(?:plughw|hw):(\d+),(\d+)", orig)
            if m:
                card, dev = m.group(1), m.group(2)
                self.conf.name_of_sound_capt = f"plughw:{card},{dev}"
                print(f"Quisk Hardware.open() - forced direct ALSA plughw for compatibility: {self.conf.name_of_sound_capt}")

        print(f"Quisk Hardware.open() - self.conf.sample_rate: {getattr(self.conf, 'sample_rate', None)}, global sample_rate: {sample_rate}")
        print(f"Quisk Hardware.open() - sending RATE command: RATE,{self.sample_rate}")
        self._set_parameter("RATE", str(self.sample_rate))
        
        # Check actual rate returned by hardware
        actual_rate = self._get_parameter("RATE")
        print(f"Quisk Hardware.open() - hardware reports active RATE is: {actual_rate}")

        # Choose initial ADC based on sample rate: PCM1808 for 48/96k, CJC5430 for 192k
        target_adc = "CJC5430" if self.sample_rate == 192000 else "PCM1808"
        print(f"Quisk Hardware.open() - sending ADC command: ADC,{target_adc}")
        self._set_parameter("ADC", target_adc)
        
        # Query hardware to get the true active ADC (resolves auto-switch override sync)
        actual_adc = self._get_parameter("ADC")
        self._adc_name = str(actual_adc).strip() if actual_adc != -1 else target_adc
        print(f"Quisk Hardware.open() - hardware reports active ADC is: {self._adc_name}")
        ...
        status_str = version + ". Capture from %s at %d Hz." % (
            self.conf.name_of_sound_capt, self.sample_rate)
        print(f"Quisk Hardware.open() finished: {status_str}")
        return status_str
    ...
    def _candidate_worst_fract(self, tune):
        ...
        return self._candidate_for_lo(tune, int(round(tune - self.sample_rate // 4)))
    ...
    def _select_comparison_pair(self, tune):
        ...
        if candidates:
            ...
            fallback = self._candidate_for_lo(tune, int(round(tune - self.sample_rate // 4)))
            pair = {
                "best": fallback,
                "middle": fallback,
                "worst": fallback,
    ...
    def set_adc(self, adc_name):
        self._set_parameter("ADC", adc_name)
        actual_adc = self._get_parameter("ADC")
        self._adc_name = str(actual_adc).strip() if actual_adc != -1 else adc_name
    ...
    def _program_candidate(self, candidate, label):
        ...
        print(f"Quisk programming SDR LO: Logical LO = {candidate['lo']:,} Hz, physical Si5351 clock = {si5351_hz:,} Hz (at {self.sample_rate} Hz sample rate)")
        self._send(
