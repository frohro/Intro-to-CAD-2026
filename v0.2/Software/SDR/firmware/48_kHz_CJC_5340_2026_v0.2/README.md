# WWU SDR — 48 kHz Fixed-Rate CJC5340 Firmware (v0.2)

**Tested hardware:** Intro-to-CAD-2026 v0.2 board (YD-RP2040 module)

> **Note:** This firmware targets the **v0.2 board** and is hardcoded for **48 kHz**.
...
48 kHz in S24\_3LE format.  A Si5351a clock synthesiser provides
...
|------|--------|-----------|-------|
| JP_M0 or GPIO22 | M0 | - | ADC Jumper — MUST BE LOW for 48 kHz |
| JP_M1 or GPIO26| M1 | - | ADC Jumper — MUST BE LOW for Single/Double Speed |
...
BCK at 48 kHz = 3.072 MHz.  PIO runs at 125 MHz
...
The device exposes **one active setting** (48 kHz) on Interface 3:
...
`snd-usb-audio` automatically selects alt 1 for the 48 kHz stream.  The 294 B
packet size on alt 1 fits comfortably through a Single-TT USB 2.0 hub.

### TinyUSB internal buffer sizing (`tusb_config.h`)

`CFG_TUD_AUDIO_FUNC_1_EP_IN_SZ_MAX` is set to **294 bytes** and
`CFG_TUD_AUDIO_FUNC_1_EP_IN_SW_BUF_SZ` to **1176 bytes**.  
...
# Should show bSubframeSize 3 and tSamFreq[ 0] 48000
```

### Quisk Configuration (`quisk_conf.py`)

This firmware is compatible with the `quisk_conf_2026.py` script.  Since the
rate is fixed at 48 kHz, ensure Quisk is configured for 48000 Hz:
...
```bash
python3 Research/48_kHz_CJC_5340_2026_v0.2/test_v0.2.py
```

Checks:
1. CDC device found (VID:PID `0xCAFE:0x4011`)
2. Quisk protocol: `VER`, `XTAL`, `MODE`, `RATE 48000`, `FREQ`
3. UAC1 enumeration — `WWU SDR` in `arecord -l`
4. USB descriptor — `bSamFreqType=1` on alt 1, 48 000 Hz, `bSubframeSize=3`
5. Audio capture at 48 kHz S24\_3LE
