...
static void send_discovery_reply(const ip_addr_t *addr, u16_t port) {
    ...
    payload[10] = 0x06; // Board ID: Hermes (OpenHPSDR Protocol 1)
...
static void handle_cc_packet(const uint8_t *data, uint16_t len) {
    ...
    // OpenHPSDR C&C data is embedded in 5-byte headers starting with 0x7F 0x7F 0x7F
    for (int offset = 8; offset <= 520; offset += 512) {
        if (data[offset] == 0x7F && data[offset+1] == 0x7F && data[offset+2] == 0x7F) {
            ...
            // Command 0x01: VFO / Tune Frequency
            if (command_type == 0x01 || (c0 & 0xFE) == 0x02) {
                uint32_t freq_hz = ((uint32_t)c1 << 24) | ((uint32_t)c2 << 16) | ((uint32_t)c3 << 8) | c4;
                if (freq_hz >= 100000 && freq_hz <= 60000000 && s_freq_cb) {
                    s_freq_cb(freq_hz);
                }
            }
...
static void init_hpsdr_packet(void) {
    ...
    s_packet_buffer[2] = HPSDR_EP6_IQ_DATA; // 0x01

    // 4-byte big-endian sequence number
    s_packet_buffer[3] = (uint8_t)(s_sequence >> 24);
    s_packet_buffer[4] = (uint8_t)(s_sequence >> 16);
    s_packet_buffer[5] = (uint8_t)(s_sequence >> 8);
    s_packet_buffer[6] = (uint8_t)(s_sequence);
    s_sequence++;

    // Subframe 1 Sync Header
    s_packet_buffer[8]  = 0x7F;
    s_packet_buffer[9]  = 0x7F;
    s_packet_buffer[10] = 0x7F;

    // Subframe 2 Sync Header
