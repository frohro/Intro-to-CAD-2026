...
static void send_discovery_reply(const ip_addr_t *addr, u16_t port) {
    ...
    if (p) {
        ...
        payload[10] = 0x01; // Board ID: 0x01 = Hermes (0x00=Metis, 0x01=Hermes, 0x06=Hermes-Lite)
...
static void handle_cc_packet(const uint8_t *data, uint16_t len) {
    ...
    // Scan for C&C sync pattern 0x7F 0x7F 0x7F anywhere in the packet
    for (int i = 0; i + 7 < (int)len; i++) {
        if (data[i] == 0x7F && data[i+1] == 0x7F && data[i+2] == 0x7F) {
            uint8_t c0 = data[i + 3];
            uint8_t c1 = data[i + 4];
            uint8_t c2 = data[i + 5];
            uint8_t c3 = data[i + 6];
            ...
            // Command 0x01 (TX/VFO Freq) or Command 0x02..0x09 (RX0..RX7 DDC Freqs)
            // Used by SDR++, Quisk, LinHPSDR, Thetis, and PowerSDR
            if (command_type >= 0x01 && command_type <= 0x09) {
                uint32_t freq_hz = ((uint32_t)c1 << 24) | ((uint32_t)c2 << 16) | ((uint32_t)c3 << 8) | c4;
                if (freq_hz >= 100000 && freq_hz <= 60000000) {
                    target_freq = freq_hz;
                }
            }
                                ...
                                const ip_addr_t *addr, u16_t port)
{
    ...
    // Check for Discovery Broadcast: 0xEFFE 0x02
    if (p->len >= 3 && data[0] == 0xEF && data[1] == 0xFE && data[2] == 0x02) {
        send_discovery_reply(addr, port);
    }
    // Start / Stop / Run command: 0xEFFE 0x04 (Len=4..64)
    // data[3]: Bit 0 = Run (1=start, 0=stop). 0x01 = Start Rx, 0x03 = Start Rx+Tx, 0x00 = Stop
    ...
    // Command and Control (C&C) or standard data: 0xEFFE 0x01
    else if (p->len >= 8 && data[0] == 0xEF && data[1] == 0xFE) {
        // Continually synchronize host IP and port to ensure active data stream
        ip_addr_copy(s_host_ip, *addr);
        s_host_port = port;
        s_active = true;
...
void openhpsdr_task(void) {
    // Background tasks / keepalive if needed
}
...
static void init_hpsdr_packet(void) {
    ...
    // Status C0 (Bit 0=PTT state, Bits 3..6=Status Address 0)
    s_packet_buffer[11] = 0x00;
    s_packet_buffer[12] = 0x00; // C1: ADC overload / sync
    s_packet_buffer[13] = 0x00; // C2: CW keyer / dash
    s_packet_buffer[14] = 0x00; // C3: User digital inputs
    s_packet_buffer[15] = 0x00; // C4: Forward power LSB
...
void openhpsdr_push_samples(const uint32_t *samples, uint32_t count) {
    ...
    for (uint32_t i = 0; i + 1 < count; i += 2) {
        ...
        uint32_t offset;
        if (s_sample_idx < 63) {
            offset = 16 + (s_sample_idx * 8); // Subframe 1 data (8 bytes per sample: 3B I, 3B Q, 2B mic)
        } else {
