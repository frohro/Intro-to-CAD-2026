/*
 * main.c -- Research: 48/96 kHz Dual-Rate UAC1 SDR
 *
 * Extends the Lab 7 firmware with runtime 48/96 kHz switching:
 *
 *   - PCM1808 256fs mode (MD1=HIGH) delivers exactly 96 kHz from the same
 *     24.576 MHz crystal (256 * 96000 = 24 576 000 Hz).
 *   - 48 kHz uses the original 512fs mode (MD1=LOW).
 ...
 *   are the PCM1808 zero-pad.  audio_task() packs to 3 bytes per sample:
 ...
 *   GPIO6   FMT  -- PCM1808 FMT (OUT, LOW = I2S standard format)
 *   GPIO8   MD1  -- PCM1808 MD1 (IN, HIGH=256fs/96kHz, LOW=512fs/48kHz)
 *   GPIO12  SDA  -- Si5351a I2C data
 *   GPIO13  SCL  -- Si5351a I2C clock
 *   GPIO9   DATA -- PCM1808 DOUT (PIO in_base)
 *   GPIO10  BCK  -- PCM1808 BCK  (PIO in_base + 1)
 *   GPIO11  WS   -- PCM1808 LRCK (PIO in_base + 2)
...
#define FMT_PIN       6    /* PCM1808 FMT   -> LOW = I2S standard fmt   */
#define MD1_PIN       8    /* PCM1808 MD1   -> LOW = Master 512fs       */
#define I2C_SDA_PIN   12   /* Si5351a SDA */
#define I2C_SCL_PIN   13   /* Si5351a SCL */
#define DATA_PIN      9    /* PCM1808 DOUT  -> PIO in_base */
#define BCK_PIN       10   /* PCM1808 BCK   -> PIO in_base + 1 */
#define WS_PIN        11   /* PCM1808 LRCK  -> PIO in_base + 2 */
...
static volatile uint32_t g_words_per_buf = 192;   /* default 96 kHz */
static volatile uint32_t g_sample_rate   = 96000U;
static volatile uint32_t g_pending_rate  = 96000U;
 ...
 *
 * Switches the PCM1808 between 48 kHz (512fs, MD1=LOW) and 96 kHz
 * (256fs, MD1=HIGH) and reconfigures the DMA ping-pong buffers via a
 * sentinel handshake with Core 1.
 *
...
{
    if (rate != 96000U) return;
    if (rate == g_sample_rate) {
        /* Even if rate is same, ensure MD1 is HIGH and words_per_buf is 192 */
        gpio_put(MD1_PIN, 1);
        g_words_per_buf = 192u;
        return;
    }

    /* Update shared state BEFORE signalling Core 1 */
    g_words_per_buf = 192u;
    g_sample_rate   = rate;

    /* PCM1808 mode: MD1=HIGH → 256fs (96 kHz) */
    gpio_put(MD1_PIN, 1);
...
static void audio_task(void)
{
    ...
    for (uint32_t i = 0; i < wpb; i++) {
        /* PCM1808 I2S (FMT=LOW) has a 1-BCK delay from WS to MSB, so the
         * raw DMA word has the dummy slot at bit 31 and D23 (sign bit) at
         * bit 30.  Left-shift by 1 moves D23 to bit 31. */
        uint32_t w = src[i] << 1;
...
/* Current sampling frequency (default 96 kHz) */
static uint32_t s_sample_rate_hz = 96000U;
                           ...
                           tusb_control_request_t const *p_request)
{
    ...
    /* With separate alt settings each alt has exactly one rate — no need
     * to consult g_pending_rate.  In this 96-kHz-only build, snd-usb-audio
     * selects alt 1 for the advertised 96 kHz rate. */
    static const uint8_t silence[1536] = {0};
    if (s_audio_alt == 1u) {
        apply_sample_rate(96000U);
        /* Pre-fill ~2 ms of silence at 96 kHz = 1536 bytes */
        tud_audio_write(silence, 1536u);
    }
    return true;
}
...
static void handle_line(const char *line, uint8_t len)
{
    ...
    if (strncmp(line, "VER", 3) == 0) {
        cdc_write("VER,SDR C firmware 2.0\r\nOK\r\n");
        return;
    }
...
int main(void)
{
    ...
    /* PCM1808 FMT=LOW -> I2S standard format (MSB delayed 1 BCK from WS) */
    gpio_init(FMT_PIN);
    gpio_set_dir(FMT_PIN, GPIO_OUT);
    gpio_put(FMT_PIN, 0);

    /* PCM1808 MD1=HIGH + MD0 tied HIGH on board -> Master 256fs mode.
     * BCK = 64 x 96000 = 6.144 MHz, LRCK = 96 kHz, SCKI = 24.576 MHz. */
    gpio_init(MD1_PIN);
    gpio_set_dir(MD1_PIN, GPIO_IN); /* Set as input to avoid driving against jumper */

    /* Force variables to match the 96 kHz jumper setting */
    g_words_per_buf = 192u;
    ...
    /* PCM1808 outputs DATA (GPIO14), BCK (GPIO15), WS (GPIO16).
     * Set these pins to PIO function and input direction across the range.
     * We enable pull-ups to prevent the I/O error if the PCM1808 is missing. */
    ...
    /* in_base = GPIO14; BCK is offset 1, WS is offset 2 */
