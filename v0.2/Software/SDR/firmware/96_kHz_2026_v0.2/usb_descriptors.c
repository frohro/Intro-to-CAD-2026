/*
 * usb_descriptors.c — Research: 96 kHz UAC1 SDR
 *
 * Configured for a single sample rate (48 or 96 kHz) controlled by
 * CFG_TUD_AUDIO_FUNC_1_SAMPLE_RATE in tusb_config.h.
 *
 * Changes from Lab 7:
 *   bSubframeSize: 4 → 3   (S24_3LE — true 24-bit, 3 bytes per sample)
 *   bBitResolution: 24      (unchanged)
 *   Alt 1: wMaxPacketSize = 582 (at 96 kHz) or 294 (at 48 kHz)
 *
 ...
 * │  Interface 3 alt 1 — Audio Streaming (Active)                     │
 * │      EP 0x81 IN  isochronous async        stereo S24_3LE          │
 * └────────────────────────────────────────────────────────────────────┘
 */
...
static const uint8_t desc_configuration[] = {
    ...
    /* Standard AS ISO EP: 96 kHz stereo S24_3LE
     * 582 bytes = (96 + 1) * 3 * 2. */
