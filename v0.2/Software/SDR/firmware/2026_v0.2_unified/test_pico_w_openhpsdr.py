...
Comprehensive Tests:
  ...
  6. Clean Stop Stream (0xEFFE 0x04 0x00).
...
def discover_pico(sock, target_ip=None):
    ...
    try:
        ...
        if len(data) >= 11 and data[0] == 0xEF and data[1] == 0xFE and data[2] == 0x02:
            ...
            # Check compliance with standard SDR software
            if board_id in (0x02, 0x06):
...
def build_cc_packet(tune_hz, sample_rate=48000, run_state=1):
    ...
    # Command 0x01: VFO tune
    buf[11] = (0x01 << 1)  # C0: Command 0x01
    ...
    # Command 0x00: MOX / Speed / Run
    buf[11+5] = 0x00  # C0: Command 0x00
...
def run_test():
    ...
    try:
        while (time.time() - start_time) < args.seconds:
            ...
            # Dynamic Retuning Test (simulates user turning VFO dial while streaming)
            ...
            seq = (data[3] << 24) | (data[4] << 16) | (data[5] << 8) | data[6]
    ...
    if total_samples > 0 and zero_samples_count == total_samples:
        ...
        print("       Quisk will calculate -inf dBFS, causing the flat line at -180 dB.")
        print("       Check that ADC DMA/PIO is feeding live audio/RF samples into openhpsdr_push_samples().")
        verdict_passed = False
